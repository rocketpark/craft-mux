(function ($) {
    ...
    Craft.MuxAssetMover = Garnish.Base.extend(
        {
            ...
            moveFolders: function (folderIds, targetFolderId, currentFolderId) {
                return new Promise((resolve, reject) => {
                    ...
                    const requests = folderIds.map((folderId) => {
                        return {
                            for: 'folder',
                            action: 'mux/folders/move-folder',
                            params: {
                                folderId,
                    ...
                    .then((totalMoved) => {
                        this._processTransferList(transferList).then((response) => {
                            ...
                            this._deleteFolders(folderIdsToDelete).then(() => {
                                // Refresh the element index to show the updated folder structure
                                Craft.elementIndex.updateElements(true);
                                resolve(totalMoved);
                            });
                        });
            ...
            _deleteFolders: function (folderIds) {
                return this._batchRequests(
                    folderIds.map((folderId) => {
                        return {
                            action: 'mux/folders/delete-folder',
                            params: { folderId },
                        };
                    })
                );
                        ...
                        Craft.sendActionRequest('POST', request.action, {
                            ...
                            .finally(() => {
                                ...
                                if (--activeRequests === 0) {
                                    ...
                                    Craft.cp.runQueue();
                                    // Refresh the element index to show any changes
                                    Craft.elementIndex.updateElements(true);
                                    resolve(responses);
                                } else {
