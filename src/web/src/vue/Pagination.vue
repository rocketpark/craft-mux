...
const page = computed(() => { return (pages.value === 0 ? 0 : state.params.page); });
const pages = computed(() => { return Math.ceil(state.total / state.params.limit); });
const prevBtnDisabled = computed(() => { return state.params.page === 1 || pages.value === 0; });
const nextBtnDisabled = computed(() => { return state.params.page === pages.value || pages.value === 0; });
