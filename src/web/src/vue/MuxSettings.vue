    ...
    <div id="settings-muxPlaybackRestrictions-field" class="field" data-attribute="muxPlaybackRestrictions">
        ...
        <div class="input ltr">
            <table id="settings-muxPlaybackRestrictions" class="editable fullwidth" style="">            
                ...
                <tbody>
                    <mux-settings-row v-for="(row, index, key) in state.restrictions" :idx="index" :restriction="row" :key="key"></mux-settings-row>
                </tbody>
            </table>
            <button 
                type="button" 
                ...
                tabindex="0" :disabled="state.loading" @click.prevent="addRow">
                    ...
                    <div class="spinner spinner-absolute" v-if="state.loading"></div>
            </button>            
        </div>
    </div>
</template>
...
<script setup>
    ...
    import { state, usePlaybackRestrictionsList, createPlaybackRestriction } from "./SettingsStore.js";
    import MuxSettingsRow from "./MuxSettingsRow.vue";
